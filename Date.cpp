...
bool Date::operator==(const Date& date) const {
  if (this->year_ == date.getYear() && this->month_ == date.getMonth()
      && this->day_ == date.getDay() && this->hour_ == date.getHour()
...
int transfer(int year, int month, int day, int hour, int minute) {
  int output;
  output = year * 100000000 + month * 1000000
...
bool Date::operator>=(const Date& date) const {
  ...
  compare = transfer(date.getYear(), date.getMonth(),
                     date.getDay(), date.getHour(), date.getMinute());
  if (thistime >= compare) {
    // std::cout << "compare" << "bigger" << std::endl;
    return true;
  }
  else
    return false;
